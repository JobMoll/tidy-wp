...
function tidy_wp_woocommerce_data(WP_REST_Request $request) {
...
if ($apiAuthOK == true) {
    ...
    $furthestDate = $furthestDate . ' 00:00:00';
    
    $closestDateShowInApp  = date($dateFormat, strtotime($closestDate));
    $furthestDateShowInApp = date($dateFormat, strtotime($furthestDate));
    
    // is closest to now
    $previousClosestDate = date("Y-m-d", strtotime($closestDate . "-1 month")) . ' ' . $currentTime;
    // is furthest away in the past
    $previousFurthestDate = date("Y-m-d", strtotime($furthestDate . "-1 month")) . ' 00:00:00';
...
add_action('rest_api_init', function () {
  register_rest_route(get_option('tidy_wp_secret_path'), 'woocommerce-data', array(
    'methods' => 'POST',
    'callback' => 'tidy_wp_woocommerce_data',
