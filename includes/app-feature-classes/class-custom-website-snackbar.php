...
if ($apiAuthOK == true) {
        ...
        if ($data->get_param('text') !== null) {
            update_option( 'tidy_wp_custom_website_snackbar_text', sanitize_text_field($data->get_param('text')), 'no' );
        }
        
        if ($data->get_param('action_text') !== null) {
            update_option( 'tidy_wp_custom_website_snackbar_action_text', sanitize_text_field($data->get_param('action_text')), 'no' );
        }
        
        if ($data->get_param('position') !== null) {
            update_option( 'tidy_wp_custom_website_snackbar_position', sanitize_text_field($data->get_param('position')), 'no' );
        }
        
        if ($data->get_param('theme') !== null) {
            update_option( 'tidy_wp_custom_website_snackbar_theme', sanitize_text_field($data->get_param('theme')), 'no' );
        }
        
        if ($data->get_param('cookie_duration') !== null) {
            update_option( 'tidy_wp_custom_website_snackbar_cookie_duration', sanitize_text_field($data->get_param('cookie_duration')), 'no' );
        }
        
        if ($data->get_param('show_duration_in_sec') !== null) {
            update_option( 'tidy_wp_custom_website_snackbar_show_duration_in_sec', sanitize_text_field($data->get_param('show_duration_in_sec')), 'no' );
        }
...
add_action( 'rest_api_init', function () {
  register_rest_route( get_option('tidy_wp_secret_path'), 'custom_website_snackbar', array(
    'methods' => 'GET',
